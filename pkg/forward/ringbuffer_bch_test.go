...
func BenchmarkRingBufferBCHPutGet(b *testing.B) {
	bufferSize := b.N / 2
	if bufferSize < 1 {
		bufferSize = 1
	}
	buf := NewRingBufferBCH(bufferSize)
	for i := 0; i < b.N; i++ {
...
func BenchmarkRingBufferBCHPut10Get5(b *testing.B) {
	bufferSize := 5
	if bufferSize < 1 {
		bufferSize = 1
	}
...
func TestRingBufferBCH(t *testing.T) {
	var b BoundedBuffer
	bufferSize := 5
	b = NewRingBufferBCH(bufferSize)
	testBuffer(t, b, bufferSize)
}

// verifies that most recent messages are kept in the buffer
func TestRingBufferBCHMostRecent(t *testing.T) {
	var tests = []struct {
		Name          string
		Size          int
		Input         []string
...
func TestRingBufferBCHBlocksWhenEmpty(t *testing.T) {
	bufferSize := 3
	buf := NewRingBufferBCH(bufferSize)
